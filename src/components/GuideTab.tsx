/*
 * The Guide tab — the full instruction manual, with the two-minute tour at
 * the top and, inside each manual section, the scenario videos for that
 * moment in a plot's life: what the developer does and what the buyer sees.
 * The "?" button's quick sheet covers the first five minutes; this covers
 * everything, grouped by when you'd reach for the app.
 */
...
/**
 * Each section lists the scenario videos that belong to it (rendered from
 * public/videos, generated by the library build). Two sections that are not
 * Code moments — reading the screen, sharing — instead link into the tour at
 * the right second. IMPORTANT: the tour timestamps must be refreshed whenever
...
const VIDEO_COUNT = Object.keys(VIDEOS).length
...
/** One scenario video: poster until played, so fifteen of them cost nothing to open the tab. */
function ScenarioVideo({ slug }: { slug: VideoSlug }) {
  const v = VIDEOS[slug]
  return (
    <div className="card scene-card" style={{ padding: 10 }}>
      <div className="row-between" style={{ margin: '0 2px 8px', alignItems: 'baseline' }}>
        <span style={{ fontWeight: 700, fontSize: 14 }}>▶ {v.title}</span>
        <span className="muted" style={{ fontSize: 12, flex: '0 0 auto' }}>
          {v.code} · {formatStamp(v.duration)}
        </span>
      </div>
      <video
...
export function GuideTab() {
  ...
  // Jump the tour to the second a section's moment appears, for the two
  // sections that are about the screen rather than a Code stage.
    ...
    <div className="content">
      ...
      <div className="card" style={{ padding: 10, marginBottom: 4 }}>
        ...
        <p className="muted" style={{ fontSize: 12.5, margin: '8px 4px 2px' }}>
          ...
          Then {VIDEO_COUNT} short videos below, one per moment in a plot's life: what you do,
          and what your buyer sees.
        </p>
      </div>
          ...
          {SECTIONS.map((s) => (
            <details key={s.title} className="guide-item">
                ...
                <span>
                  ...
                  {s.videos && (
                    <span className="muted" style={{ fontWeight: 400, fontSize: 12 }}>
                      {' '}
                      · {s.videos.length === 1 ? '1 video' : `${s.videos.length} videos`}
                    </span>
                  )}
                </span>
              ...
              <div className="guide-body">
                ...
                {s.videos?.map((slug) => (
                  <ScenarioVideo key={slug} slug={slug} />
                ))}
                {s.body.map((p, i) => (
                  <p key={i}>{p}</p>
                ))}
      ...
      <p className="muted" style={{ fontSize: 12, marginTop: 20, lineHeight: 1.55 }}>
        ...
        the rules behind any deadline, see The Code tab — or tap "why?" wherever you see it.
      </p>
    </div>
  )
}
