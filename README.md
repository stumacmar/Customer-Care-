...
- **Phase 5 (done, serverless)** — the buyer-side app, with no backend. "Share with buyer" on a plot generates a private link whose **fragment carries the snapshot** (fragments are never sent to the server, so the data goes device-to-device). The buyer's view — installable at the same address — shows their journey, plain-English rights (cooling-off, PCI, complaint timetable, NHOS), documents received, choices and changes, and issue status with the response deadlines they're entitled to. "Report a problem" produces a pre-addressed email carrying a paste-code; the developer pastes it into "Paste a report from the buyer's app" and it logs with the correct clock. The buyer keeps their own local evidence trail of every report sent.
- **Phase 6 (the server decision)** — live sync instead of link snapshots, push notifications, multi-developer accounts, nominated-representative access, photos in buyer reports, and Code-body integration. All gated on adding a backend + accounts (hosting, GDPR controllership, cost) — a product decision, not a feature.
